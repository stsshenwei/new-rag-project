...
| Change retrieval or feedback logic | `backend/app/services/rag_service.py` |
| Change embedding or vector persistence | `backend/app/services/vector_store.py` |
| Add support for a new file type | `backend/app/services/document_loader.py` |
| Change upload or parse behavior | `backend/app/main.py`, `backend/app/schemas.py`, `backend/app/services/rag_service.py`, `backend/app/services/document_loader.py` |
...
## Agent Runtime Grep-First Notes

Intelligent reasoning mode defaults to LLM-driven grep-first retrieval when `AGENT_RUNTIME_ENABLED=true`.

- `REASONING_LLM_GREP_FIRST_ENABLED=true` makes reasoning-mode factual KB questions prefer `grep_chunks` before semantic-only retrieval or final answers.
...
- Search terms generated by the LLM are retrieval hints only. Final answers still require retrieved and deep-read KB evidence.

Focused validation:

```powershell
cd backend
.\.venv\Scripts\python.exe -m unittest tests.test_agent_runtime_loop tests.test_agent_runtime_prompts_tools -v
```
